...
class PCAFactorStrategy:
    ...
    def compute_pca_for_rebalance(self, rebalance_date):
        ...
        # Rescale loadings back to original log return space
        num_stocks = min(len(self.stocks), loadings.shape[0])
        stock_std = returns_std.values[:num_stocks]
        loadings = loadings / stock_std[:, np.newaxis] # Fixed: divide instead of multiply
    ...
    def compute_actual_returns_for_rebalance(self, rebalance_date):
        ...
        try:
            ...
            if np.any(np.abs(returns) > np.log(1.5)):  # Log of 1.5 ≈ 0.405, i.e., 50% return
    ...
    def compute_predicted_pc_movement(self, rebalance_date):
        ...
        for pc_idx in range(self.num_pcs):
            ...
            if pred_value is not None:
                pred_pct_change_pc[pc_idx] = pred_value # Fixed: removed unnecessary scaling by sigma
    ...
    def compute_predicted_stock_returns(self, rebalance_date):
        ...
        # Compute predicted stock returns (log returns)
        r_hat = pca_matrix @ pred_pct_change_pc
        stock_std = self.rebalance_data.get(rebalance_date, {}).get('stock_std')
        r_hat = stock_std[:num_stocks] * r_hat * stock_std[:num_stocks] # Fixed: multiply by stock_std twice to match scales
        return r_hat
    ...
    def plot_pred_vs_actual_scatter(self):
        ...
        for rebalance_date in self.rebalance_dates[:-1]: # Skip last if no actuals
            ...
            if r_hat_weighted is not None and actual_returns is not None:
                predicted_data[rebalance_date] = r_hat_weighted
                actual_data[rebalance_date] = actual_returns
        ...
        ax.set_xlabel('Weekly Predicted Returns (Weighted)')
        ax.set_ylabel('Weekly Actual Returns')
        ax.set_title('Scatter: Predicted vs Actual Stock Returns Across Rebalance Dates')
    ...
    def plot_predicted_over_time(self):
        ...
        ax.set_ylabel('Predicted Stock Movement (Weighted Returns)')
    ...
    def plot_actual_over_time(self):
        ...
        ax.set_xlabel('Time (Rebalance Dates)')
        ax.set_ylabel('Actual Stock Movement (Returns)')
    ...
    def compute_betas(self, rebalance_date):
        ...
        # Compute log returns for stocks and sector
        returns = self.compute_log_returns(prices)
        sector_returns = self.compute_log_returns(sector_data)
    ...
    def compute_portfolio_metrics(self, rebalance_date, weights, prev_weights=None):
        ...
        # Calculate P&L: weights (in dollars) * actual_returns (log returns)
        # Convert log returns to simple returns for P&L calculation: exp(log_return) - 1
        simple_returns = np.expm1(actual_returns)
        pnl = np.sum(weights * simple_returns)
    ...
    def debug_and_validate_last_two_days(self):
        ...
        for date in last_two_dates:
            ...
            if actual_returns is not None:
                ...
                if np.any(np.abs(actual_returns) > np.log(1.5)):
