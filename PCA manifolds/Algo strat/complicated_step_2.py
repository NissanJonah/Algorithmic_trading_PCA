...
class PCAFactorStrategy:
    def __init__(self, stocks, start_date, end_date, lookback=252, initial_capital=10000):
        ...
        self.factors = ['XLI', '^TNX', '^GSPC', 'UUP', 'IYT', 'DIA', 'COPX', 'XME', '^VIX', 'GLD', 'CAT', 'HON', 'XLB',
                        'XLF', 'XLE', 'DE', 'XLU', 'GDX', 'RTX']
        ...
        self.total_regression_count = 0  # Track total regression lines made
    ...
    def factor_regression(self, pc_series, factor_changes, rebalance_date):
        ...
        for pc in pc_series_full.columns:
            ...
            # Correlation filter
            min_corr = 0.05 if pc == 'PC_1' else 0.1
            correlations = pd.Series(np.abs(np.corrcoef(X_scaled.T, y_scaled)[:-1, -1]), index=self.factors)
            available_factors = correlations[correlations > min_corr].index.tolist()
            if not available_factors:
                self.r2_history[pc].append((rebalance_date, 0.0))
                self.r2_training_history[pc].append((rebalance_date, 0.0))
                continue
            model = LinearRegression()
            max_factors = min(len(available_factors), len(common_period_dates) - 2, 2)  # Limit to 2 factors
            if pc == 'PC_1':
                # PC_1: Up to 2 factors with multicollinearity check
                best_subset = [correlations[available_factors].idxmax()]
                if len(available_factors) > 1 and max_factors > 1:
                    for f in correlations[available_factors].index[1:]:
                        if all(abs(factor_changes_full[f].corr(factor_changes_full[bf])) < 0.75 for bf in best_subset):
                            best_subset.append(f)
                            break
                        if len(best_subset) >= max_factors:
                            break
            else:
                # PC_2–PC_5: Stepwise forward selection (up to 2 factors)
                best_subset = []
                best_r2 = -float('inf')
                # Step 1: Select the best single factor
                for f in available_factors:
                    X_subset = factor_changes_full[[f]].to_numpy()
                    model.fit(X_subset, y_full)
                    r2 = model.score(X_subset, y_full)
                    adjusted_r2 = 1 - (1 - r2) * (len(y_full) - 1) / (len(y_full) - 1 - 1)
                    if adjusted_r2 > best_r2:
                        best_r2 = adjusted_r2
                        best_subset = [f]
                # Step 2: Try adding a second factor
                if len(available_factors) > 1 and max_factors > 1:
                    for f in available_factors:
                        if f not in best_subset and all(abs(factor_changes_full[f].corr(factor_changes_full[bf])) < 0.75 for bf in best_subset):
                            temp_subset = best_subset + [f]
                            X_subset = factor_changes_full[temp_subset].to_numpy()
                            model.fit(X_subset, y_full)
                            r2 = model.score(X_subset, y_full)
                            adjusted_r2 = 1 - (1 - r2) * (len(y_full) - 1) / (len(y_full) - len(temp_subset) - 1)
                            if adjusted_r2 > best_r2:
                                best_r2 = adjusted_r2
    ...
    def validate_step2(self):
        ...
        for rebalance_date in self.rebalance_dates:
            ...
            # Count regression lines for this rebalance date
            self.total_regression_count += len(regression_results)
        ...
        # Print total regression lines made
        print(f"Total linear regression lines made: {self.total_regression_count}")
        ...
        # Add average R² for each PC at different lookback periods
        print(f"\nAverage R² for each PC at {self.lookback}-day lookback:")
        for pc in [f'PC_{i + 1}' for i in range(min(5, len(self.r2_history)))]:
            if self.r2_training_history[pc] and self.r2_history[pc]:
                training_r2s = [r2 for _, r2 in self.r2_training_history[pc]]
                rebalancing_r2s = [r2 for _, r2 in self.r2_history[pc]]
                avg_training = np.mean(training_r2s)
                avg_rebalancing = np.mean(rebalancing_r2s)
                print(f"  {pc}: Training R² = {avg_training:.4f}, Rebalancing R² = {avg_rebalancing:.4f}")
...
if __name__ == "__main__":
    stocks = ['GE', 'CAT', 'RTX', 'UNP', 'HON', 'BA', 'DE', 'UPS', 'LMT', 'ETN', 'PH', 'ITW', 'GD', 'NOC', 'MMM', 'EMR', 'CMI', 'ROK', 'CSX', 'WM', 'IR', 'NSC', 'FAST', 'PCAR']
