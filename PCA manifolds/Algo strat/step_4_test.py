...
class PCAFactorStrategy:
    ...
    def factor_regression(self, pc_series, factor_changes, rebalance_date, std_returns, V, scaler):
        ...
        try:
            ...
            if len(common_dates) < 30:
                print(f"Skipping regression for {rebalance_date}: Insufficient data ({len(common_dates)} < 30)")
                return {}
        ...
        for pc in pc_series_window.columns:
            ...
            X_raw = factor_changes_window.to_numpy()

            if np.any(np.isnan(y)) or np.any(np.isnan(X_raw)) or len(y) < 40:
                print(f"Skipping regression for {pc}: NaN values or insufficient samples ({len(y)})")
            ...
            correlations = np.abs(
                [np.corrcoef(X_raw[:, i], y)[0, 1] for i in range(X_raw.shape[1]) if not np.all(np.isnan(X_raw[:, i]))])
            ...
            # Get the final set of non-zero coefficient factors
            ...
            # Subset to non-zero coefficient features
            ...
            # Re-fit scaler on the final set of features
            scaler_X_final = StandardScaler()
            X_final_scaled = scaler_X_final.fit_transform(X_final)

            # Re-fit Lasso on the final scaled features (optional, but ensures consistency)
            lasso_final = LassoCV(
                alphas=settings['alphas'],
                cv=settings['cv'],
                max_iter=settings['max_iter'],
            ...
            # Update betas for the final factors
            betas = {final_factors[i]: lasso_final.coef_[i] for i in range(len(final_factors))}

            # Compute prediction R²
            prediction_r2 = 0.0
            try:
                future_dates = self.data.index[self.data.index > rebalance_date][:15]
                if len(future_dates) >= 10:
                    ...
                    if not future_returns.empty and len(future_returns) >= 10:
                        ...
                        if len(common_dates) >= 8:
                            ...
                            X_raw_pred = future_factor_changes.loc[common_dates, final_factors].values
                            X_pred = scaler_X_final.transform(X_raw_pred)
            ...
            results[pc] = {
                ...
                'model_type': 'lasso',
                'scaler_X': scaler_X_final  # Store scaler fitted on final features
            }
    ...
    def compute_predictions(self, pc_series, factor_changes, regression_results, rebalance_date, V, u_centrality):
        ...
        # print(f"DEBUG - Stocks: {self.stocks}")
        ...
        # print(f"DEBUG - PC series date range: {pc_series.index[0]} to {pc_series.index[-1]}")
        # print(f"DEBUG - Lookback period L: {self.lookback}")

        try:
            rebalance_idx = pc_series.index.get_loc(rebalance_date)
            # print(f"DEBUG - Rebalance date index in PC series: {rebalance_idx}")

            # Use rolling window std with the lookback period on the existing PC series
            ...
            # print(f"DEBUG - sigma_pc_vec shape: {sigma_pc_vec.shape}")
            ...
            if np.any(np.isnan(sigma_pc_vec)):
                ...
                # print(f"DEBUG - Fallback sigma_pc_vec: {sigma_pc_vec}")
        ...
        try:
            ...
            if current_idx < len(self.rebalance_dates) - 1:
                ...
                days_to_next_rebalance = (next_rebalance_date - rebalance_date).days
            else:
                days_to_next_rebalance = 5
        except (ValueError, IndexError):
            days_to_next_rebalance = 5

        # print(f"\nDEBUG - Days to next rebalance: {days_to_next_rebalance}")

        # Step 2: Predict PC movement using regression
        # print(f"\nStep 2: Computing PC Movement Predictions")
        ...
        for pc_idx, pc in enumerate(pc_series.columns):
            if pc in regression_results:
                ...
                # print(f"DEBUG - Alpha: {res['alpha']}")
                # print(f"DEBUG - Beta coefficients: {res['beta']}")
                ...
                try:
                    ...
                    # print(f"DEBUG - Current factor changes: {dict(zip(best_factors, X_current))}")

                    delta_pc_pred = res['alpha'] + np.dot(list(res['beta'].values()), X_current)
                    delta_pc_pred_vec[pc_idx] = delta_pc_pred * np.sqrt(days_to_next_rebalance)
                    # print(
                    #     f"DEBUG - PC prediction (scaled to {days_to_next_rebalance} days): {delta_pc_pred_vec[pc_idx]}")
        ...
        print(f"\nDEBUG - delta_pc_pred_vec (scaled to period): {delta_pc_pred_vec}")

        # Step 3: Convert to % movement: PredictedPctChange_PC_k = ΔPC_pred_k * σ_PC_k
        # print(f"\nStep 3: Converting to Percentage Movement")
        predicted_pct_change_vec = delta_pc_pred_vec * sigma_pc_vec
        predicted_pct_change_vec = np.clip(predicted_pct_change_vec, -0.1, 0.1)  # Cap at ±10%
        # print(f"DEBUG - predicted_pct_change_vec (ΔPC_pred ⊙ σ_PC): {predicted_pct_change_vec}")

        # Step 4: Compute stock returns: r_hat = V · (ΔPC_pred ⊙ σ_PC)
        # print(f"\nStep 4: Computing Stock Returns")
        # print(f"DEBUG - V matrix (first 3 rows): \n{V[:3, :]}")
        # print(f"DEBUG - predicted_pct_change_vec: {predicted_pct_change_vec}")

        r_hat_base = V @ predicted_pct_change_vec
        # print(f"DEBUG - r_hat_base (V @ predicted_pct_change_vec): {r_hat_base}")
        # print(f"DEBUG - r_hat_base shape: {r_hat_base.shape}")
        #
        # # Step 5: Apply centrality weighting
        # print(f"\nStep 5: Applying Centrality Weighting")
        # print(f"DEBUG - u_centrality: {u_centrality}")
        # print(f"DEBUG - u_centrality shape: {u_centrality.shape}")
        # print(f"DEBUG - u_centrality mean: {np.mean(u_centrality)}, std: {np.std(u_centrality):.2f}")

        r_hat_weighted = r_hat_base * u_centrality
        # print(f"DEBUG - r_hat_weighted (final): {r_hat_weighted}")

        # Summary for this rebalance period
        # print(f"\nSUMMARY for {rebalance_date.date()}:")
        # print(f"Period length: {days_to_next_rebalance} days")
        for i in range(min(5, k)):
            pc_name = f"PC_{i + 1}"
            # print(
            #     f"{pc_name}: σ={sigma_pc_vec[i]:.6f}, ΔPC_period={delta_pc_pred_vec[i]:.6f}, PctChange={predicted_pct_change_vec[i]:.6f}")

        # print(f"Stock predictions (r_hat_weighted):")
        for stock, ret in zip(self.stocks, r_hat_weighted):
            print(f"  {stock}: {ret:.6f}")

        return sigma_pc_vec, delta_pc_pred_vec, predicted_pct_change_vec, r_hat_base, r_hat_weighted
    def compute_actual_returns(self, rebalance_date):
        ...
        try:
            ...
            if current_idx < len(self.rebalance_dates) - 1:
                ...
                days_to_next_rebalance = 5
                # For the last rebalance, get 5 trading days after
                future_dates = self.data.index[self.data.index > rebalance_date][:6]
                if len(future_dates) == 0:
                    return None, 0
                end_date = future_dates[-1] if len(future_dates) >= 5 else future_dates[-1]
        except (ValueError, IndexError):
            days_to_next_rebalance = 5
            future_dates = self.data.index[self.data.index > rebalance_date][:6]
            if len(future_dates) == 0:
        ...
        # print(f"DEBUG - Expected period: {days_to_next_rebalance} days")
        ...
        try:
            ...
            #
            # print(f"DEBUG - Start prices (first 5): {start_price.head().to_dict()}")
            # print(f"DEBUG - End prices (first 5): {end_price.head().to_dict()}")

            # Compute cumulative log return: log(P_end / P_start)
            actual_returns = np.log(end_price / start_price).values
            actual_days = (end_date - rebalance_date).days
            #
            # print(f"DEBUG - Actual period length: {actual_days} days")
            # print(f"DEBUG - Actual returns: {actual_returns}")

            return actual_returns, actual_days
    ...
    def validate_step4(self):
        ...
        Print detailed diagnostics for the prediction process.
        """
        print("=" * 80)
        ...
        for i, rebalance_date in enumerate(self.rebalance_dates):
            ...
            # Detailed debugging only for last 3 dates
            # if rebalance_date in debug_dates:
                # print(f"\nDETAILED DEBUG for {rebalance_date.date()}:")
                # print(f"Lookback period: {lookback_start.date()} to {rebalance_date.date()}")
                # print(f"Returns shape: {returns.shape}")
                # print(f"Standardized returns mean: {std_returns.mean().mean():.6f}")
                # print(f"PC explained variance: {explained_var[:5]}")
                # print(f"Regression results available for PCs: {list(regression_results.keys())}")

            # Compute predictions with detailed debugging
            sigma_pc_vec, delta_pc_pred_vec, predicted_pct_vec, r_hat, r_hat_weighted = self.compute_predictions(
                pc_series, factor_changes, regression_results, rebalance_date, V, u_centrality
            )

            # Compute actual returns for the same period
            actual_returns, actual_days = self.compute_actual_returns(rebalance_date)

            # Store results
            if r_hat_weighted is not None and actual_returns is not None:
                # print(f"\nCOMPARISON for {rebalance_date.date()}:")
                # print(
                #     f"Prediction period: Expected {delta_pc_pred_vec is not None and 'days' or '5'} days, Actual: {actual_days} days")
        ...
        if predicted_actual_pairs:
            ...
            period_stats = df_pairs.groupby('period_days').agg({
                'predicted': 'count',
                'actual': lambda x: np.corrcoef(df_pairs.loc[x.index, 'predicted'], x)[0, 1] if len(x) > 1 else np.nan
            }).rename(columns={'predicted': 'count', 'actual': 'correlation'})
            #
            # print(f"\nPERIOD LENGTH ANALYSIS:")
            # print(period_stats)
            avg_pred_abs = np.mean(np.abs(r_hat_weighted))
            avg_actual_abs = np.mean(np.abs(actual_returns))
            # print(f"Average absolute predicted return: {avg_pred_abs:.6f}")
            # print(f"Average absolute actual return: {avg_actual_abs:.6f}")
