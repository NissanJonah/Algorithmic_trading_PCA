...
def analyze_prediction_vs_actual_detailed(strategy):
    ...
    print(f"Predicted returns - Mean absolute: {pred_mean:.6f}")
    print(f"Actual returns - Mean absolute: {actual_mean:.6f}")

    # Avoid arbitrary rescaling; only rescale if predicted returns are clearly off
    if pred_mean > 1.0:
    ...
    if abs(slope - 1.0) > 0.5:
        ...
        print("✓ Slope close to 1.0 - appropriate scaling")

    if abs(intercept) > np.std(actual_clean) * 0.1:
        print("⚠️  Large intercept - predictions may be biased")
    else:
        print("✓ Small intercept - predictions appear unbiased")
