...
class PCAFactorStrategy:
    ...
    def __init__(self, start_date, end_date, rebalance_frequency='W-FRI', lookback=252,
        ...
        self.avg_predicted_over_time = []  # Average predicted return per rebalance date
        self.avg_actual_over_time = []  # Average actual return per rebalance date
        self.significant_predicted = []  # Predicted returns for significant movements
        self.significant_actual = []  # Actual returns for significant movements
    ...
    def compute_predicted_stock_returns(self, rebalance_date):
        """Compute predicted stock returns using loadings and weighted predicted PC movements."""
        pred_pct_change_pc = self.compute_predicted_pc_movement(rebalance_date)
        if pred_pct_change_pc is None:
            return None

        pca_matrix = self.rebalance_data.get(rebalance_date, {}).get('pca_matrix')
        explained_variance = self.rebalance_data.get(rebalance_date, {}).get('explained_variance')

        if pca_matrix is None or explained_variance is None:
            return None

        # Get regression results for this date to extract R² values
        regression_results = self.regression_results.get(rebalance_date, {})
        ...
        explained_variance = explained_variance[:num_pcs]

        # Calculate weights for each PC
        pc_weights = np.ones(num_pcs)  # Default to equal weights

        for pc_idx in range(num_pcs):
            # Component 1: Explained variance (how important this PC is structurally)
            variance_weight = explained_variance[pc_idx]

            # Component 2: Regression R² (how well we can predict this PC)
            if pc_idx in regression_results and 'cv_r2_mean' in regression_results[pc_idx]:
                # Use cross-validation R² as it's more robust than training R²
                prediction_quality = max(0, regression_results[pc_idx]['cv_r2_mean'])
            else:
                prediction_quality = 0  # No prediction quality if regression failed

            # Combined weight: variance importance × prediction quality
            # Add small epsilon to avoid zero weights
            pc_weights[pc_idx] = variance_weight * prediction_quality + 1e-6

        # Normalize weights so they sum to 1
        pc_weights = pc_weights / np.sum(pc_weights)

        # Apply weights to PC predictions
        weighted_pred_pct_change_pc = pred_pct_change_pc * pc_weights

        # Compute weighted predicted stock returns
        r_hat = pca_matrix @ weighted_pred_pct_change_pc

        # Print weighting information for transparency
        #print(f"PC Weights for {rebalance_date.date()}:")
        for pc_idx in range(num_pcs):
            variance_pct = explained_variance[pc_idx] * 100
            r2_pct = regression_results.get(pc_idx, {}).get('cv_r2_mean',
                                                            0) * 100 if pc_idx in regression_results else 0
            weight_pct = pc_weights[pc_idx] * 100
            #print(f"  PC{pc_idx + 1}: Variance={variance_pct:.1f}%, R²={r2_pct:.1f}%, Weight={weight_pct:.1f}%")
    ...
    def compute_comprehensive_metrics(self):
        ...
        # Lists to store returns for each regime
        high_vol_returns = []
        low_vol_returns = []
        bull_returns = []
        bear_returns = []

        high_vol_long_returns = []
        low_vol_long_returns = []
        bull_long_returns = []
        bear_long_returns = []

        high_vol_short_returns = []
        low_vol_short_returns = []
        bull_short_returns = []
        bear_short_returns = []
        ...
        for rebalance_date in self.rebalance_dates[:-1]:
            r_hat = self.compute_predicted_stock_returns(rebalance_date)
            actual_returns = self.rebalance_data.get(rebalance_date, {}).get('actual_returns')
            weights = self.portfolio_history.get(rebalance_date, {})

            if r_hat is not None and actual_returns is not None and weights:
                ...
                for i, stock in enumerate(self.stocks[:min_len]):
                    if stock in weights:
                        weight = weights[stock]
                        ret = actual_trimmed[i]
                        # Skip extreme returns to prevent outliers
                        if abs(ret) > 10:  # Assuming returns > 1000% are errors (adjust threshold as needed)
                            continue
                ...
                # Normalize returns
                portfolio_return = portfolio_return / total_weight if total_weight > 0 else 0
                long_return = long_return / long_weight if long_weight > 0 else 0
                short_return = short_return / short_weight if short_weight > 0 else 0
                ...
                if min_len > 1:
                    ...
                    if market_vol > np.median([np.std(self.rebalance_data.get(d, {}).get('actual_returns', []))
                        ...
                        high_vol_returns.append(portfolio_return)
                        high_vol_long_returns.append(long_return)
                        high_vol_short_returns.append(short_return)
                    else:
                        low_vol_periods.append((pearson_corr, hit_rate, mae))
                        low_vol_returns.append(portfolio_return)
                        low_vol_long_returns.append(long_return)
                        low_vol_short_returns.append(short_return)

                    if market_return > 0:
                        bull_periods.append((pearson_corr, hit_rate, mae))
                        bull_returns.append(portfolio_return)
                        bull_long_returns.append(long_return)
                        bull_short_returns.append(short_return)
                    else:
                        bear_periods.append((pearson_corr, hit_rate, mae))
                        bear_returns.append(portfolio_return)
                        bear_long_returns.append(long_return)
                        bear_short_returns.append(short_return)
        ...
        return {
            ...
            'high_vol_long_returns': high_vol_long_returns,
            'low_vol_long_returns': low_vol_long_returns,
            'bull_long_returns': bull_long_returns,
            'bear_long_returns': bear_long_returns,
            'high_vol_short_returns': high_vol_short_returns,
            'low_vol_short_returns': low_vol_short_returns,
            'bull_short_returns': bull_short_returns,
            'bear_short_returns': bear_short_returns
        }
    ...
    def _print_enhanced_summary_statistics(self):
        ...
        if len(metrics['high_vol_periods']) > 0 and len(metrics['low_vol_periods']) > 0:
            ...
            high_vol_avg_return = np.mean(metrics['high_vol_returns']) * 100 if metrics['high_vol_returns'] else 0
            low_vol_avg_return = np.mean(metrics['low_vol_returns']) * 100 if metrics['low_vol_returns'] else 0
            high_vol_avg_long = np.mean(metrics['high_vol_long_returns']) * 100 if metrics[
                'high_vol_long_returns'] else 0
            low_vol_avg_long = np.mean(metrics['low_vol_long_returns']) * 100 if metrics['low_vol_long_returns'] else 0
            high_vol_avg_short = np.mean(metrics['high_vol_short_returns']) * 100 if metrics[
                'high_vol_short_returns'] else 0
            low_vol_avg_short = np.mean(metrics['low_vol_short_returns']) * 100 if metrics[
                'low_vol_short_returns'] else 0

            print(f" High Volatility Periods:")
            print(f"   Correlation: {high_vol_corr:.4f}")
            print(f"   Hit Rate: {high_vol_hit:.4f}")
            print(f"   Average Return: {high_vol_avg_return:.4f}%")
            print(f"   Average Long Return: {high_vol_avg_long:.4f}%")
            print(f"   Average Short Return: {high_vol_avg_short:.4f}%")
            ...
            print(f"   Average Return: {low_vol_avg_return:.4f}%")
            print(f"   Average Long Return: {low_vol_avg_long:.4f}%")
            print(f"   Average Short Return: {low_vol_avg_short:.4f}%")
        ...
        if len(metrics['bull_periods']) > 0 and len(metrics['bear_periods']) > 0:
            ...
            bull_avg_return = np.mean(metrics['bull_returns']) * 100 if metrics['bull_returns'] else 0
            bear_avg_return = np.mean(metrics['bear_returns']) * 100 if metrics['bear_returns'] else 0
            bull_avg_long = np.mean(metrics['bull_long_returns']) * 100 if metrics['bull_long_returns'] else 0
            bear_avg_long = np.mean(metrics['bear_long_returns']) * 100 if metrics['bear_long_returns'] else 0
            bull_avg_short = np.mean(metrics['bull_short_returns']) * 100 if metrics['bull_short_returns'] else 0
            bear_avg_short = np.mean(metrics['bear_short_returns']) * 100 if metrics['bear_short_returns'] else 0

            print(f" Bull Market Periods:")
            print(f"   Correlation: {bull_corr:.4f}")
            print(f"   Hit Rate: {bull_hit:.4f}")
            print(f"   Average Return: {bull_avg_return:.4f}%")
            print(f"   Average Long Return: {bull_avg_long:.4f}%")
            print(f"   Average Short Return: {bull_avg_short:.4f}%")
            ...
            print(f"   Average Return: {bear_avg_return:.4f}%")
            print(f"   Average Long Return: {bear_avg_long:.4f}%")
            print(f"   Average Short Return: {bear_avg_short:.4f}%")
    ...
    def rebalance_portfolio(self, rebalance_date):
        ...
        # Calculate total capital that could be deployed (current capital)
        total_capital_available = self.current_capital

        # Calculate total capital actually deployed (sum of absolute weights)
        total_capital_deployed = np.sum(np.abs(weights)) if weights is not None else 0

        # Print the two new statements
        print(f"Total capital that could be deployed on {rebalance_date.date()}: ${total_capital_available:,.2f}")
        print(f"Total capital actually deployed on {rebalance_date.date()}: ${total_capital_deployed:,.2f}")
    ...
    def print_portfolio_metrics(self):
        ...
        # Basic metrics - Convert P&L to percentage returns
        dates = sorted(self.portfolio_values.keys())
        portfolio_values_list = [self.portfolio_values[date] for date in dates]

        # Calculate percentage returns from portfolio values
        returns = []
        for i in range(1, len(portfolio_values_list)):
            ret = (portfolio_values_list[i] - portfolio_values_list[i - 1]) / portfolio_values_list[i - 1]
            returns.append(ret)
        ...
        final_value = portfolio_values_list[-1]
        ...
        for date in dates[:-1]:  # Skip last date
            if date in self.portfolio_history:
                portfolio = self.portfolio_history[date]
        ...
        print(f"\n6. ADDITIONAL RISK METRICS:")
        if len(returns) > 1:
            # Maximum drawdown - calculate from portfolio values
            portfolio_values_array = np.array(portfolio_values_list)
            running_max = np.maximum.accumulate(portfolio_values_array)
            drawdown = (portfolio_values_array - running_max) / running_max
    ...
    def main(self):
        ...
        for i, rebalance_date in enumerate(self.rebalance_dates):
            ...
            # Print the dates used for prediction inputs (X_next) to expose look-ahead bias
            if self.regression_results[rebalance_date] is not None:
                ...
                if not factor_returns.empty:
                    training_start = factor_returns.index[0].date()
                    training_end = factor_returns.index[-1].date()
                    print(f"Regression training period: {training_start} to {training_end}")
                else:
                    print("No factor returns available for training")
            ...
            self.update_comparison_metrics(rebalance_date)
        ...
        self.plot_predicted_vs_actual_over_time_comparison()

        # Around line 1870 in your main function, after self._print_enhanced_summary_statistics()

    def plot_training_r2_scores(self):
        """Create a plot of Rolling Training R² for each principal component over time."""
        train_r2_data = {i: [] for i in range(self.num_pcs)}
        dates = []

        print("\nPreparing data for Training R² plot...")
        for rebalance_date in self.rebalance_dates:
            dates.append(rebalance_date)
            for pc_idx in range(self.num_pcs):
                train_r2 = self.compute_rolling_train_r2(pc_idx, rebalance_date, window=10)
                train_r2_data[pc_idx].append(train_r2 if train_r2 is not None else np.nan)
                print(f"PC{pc_idx + 1} at {rebalance_date.date()}: Train R²={train_r2}")

        colors = ['blue', 'red', 'green', 'orange', 'purple']
        fig, ax = plt.subplots(figsize=(12, 5))

        # Plot Rolling Training R²
        print("\nPlotting Training R²...")
        for pc_idx in range(self.num_pcs):
            valid_mask = ~np.isnan(train_r2_data[pc_idx])
            valid_dates = np.array(dates)[valid_mask]
            valid_r2 = np.array(train_r2_data[pc_idx])[valid_mask]
            print(f"PC{pc_idx + 1}: {len(valid_r2)} valid training R² points")
            if len(valid_r2) > 0:
                ax.plot(valid_dates, valid_r2, label=f'PC{pc_idx + 1}', marker='o', markersize=3,
                        color=colors[pc_idx % len(colors)], alpha=0.7)
            else:
                print(f"Warning: No valid training R² data for PC{pc_idx + 1}")

        ax.set_title("Rolling Training R² Over Time", fontsize=14)
        ...
        # Output average R² over time
        print("\nAverage Rolling Training R² over time:")
        for pc_idx in range(self.num_pcs):
            avg_r2 = np.nanmean(train_r2_data[pc_idx])
            print(f"PC{pc_idx + 1}: {avg_r2:.4f}")
    ...
    def compute_rolling_r2(self, pc_idx, rebalance_date, window=10, use_future=False):
        ...
        # Debugging: Check available data
        print(f"\nComputing rolling R² for PC{pc_idx + 1} at {rebalance_date.date()}:")
        print(f"  Available prediction dates: {len(self.predictions[pc_idx].keys())}")
        print(f"  Valid dates (< {rebalance_date.date()}): {len(valid_dates)}")
        print(f"  Actuals available: {len(actuals_dict[pc_idx].keys())}")

        if len(valid_dates) < 2:
            print(f"  Warning: Insufficient valid dates ({len(valid_dates)} < 2)")
            return None

        valid_dates = sorted(valid_dates)[-window:] if len(valid_dates) >= window else sorted(valid_dates)
        print(f"  Using dates: {[d.date() for d in valid_dates]}")

        preds = []
        actuals = []
        for d in valid_dates:
            if d in self.predictions[pc_idx] and d in actuals_dict[pc_idx]:
                pred = self.predictions[pc_idx][d]
                act = actuals_dict[pc_idx][d]
                if pred is not None and act is not None and not np.isnan(pred) and not np.isnan(act):
                    preds.append(pred)
                    actuals.append(act)

        print(f"  Valid predictions: {len(preds)}")
        print(f"  Valid actuals: {len(actuals)}")

        if len(preds) < 2:
            print(f"  Warning: Insufficient valid data points ({len(preds)} < 2)")
            return None

        if np.var(actuals) < 1e-10:
            print(f"  Warning: Near-zero variance in actuals ({np.var(actuals)})")
            return None

        try:
            r2 = r2_score(actuals, preds)
            print(f"  Computed R²: {r2:.4f}")
            return max(r2, -10.0)
        except Exception as e:
    ...
    def train_regression_models(self, rebalance_date):
        """Train regression models by selecting top 1-3 factors with highest R² for each PC."""
        pc_returns, next_pc_returns = self.compute_weekly_pc_returns(rebalance_date)

        if pc_returns is None or pc_returns.empty:
            print(f"Warning: No PC returns for {rebalance_date.date()}")
            return None

        # Get factor returns aligned with PC returns
        factor_returns = self.factor_data.loc[self.factor_data.index <= rebalance_date]
        factor_returns = factor_returns.iloc[-self.regression_lookback:]

        if factor_returns.empty:
            print(f"Warning: No factor returns for {rebalance_date.date()}")
            return None

        # Align dates between PC returns and factor returns
        common_dates = pc_returns.index.intersection(factor_returns.index)
        if len(common_dates) < self.min_regression_weeks:
            print(f"Warning: Insufficient common dates for {rebalance_date.date()}")
            return None
        ...
        for pc_idx in range(min(self.num_pcs, pc_returns_aligned.shape[1])):
            y = pc_returns_aligned.iloc[:, pc_idx].values

            # Step 1: Calculate R² between each factor and the PC
            factor_r2_scores = {}
            for factor in factor_returns_aligned.columns:
                X_factor = factor_returns_aligned[factor].values.reshape(-1, 1)

                # Remove any NaN values
                valid_mask = ~(np.isnan(X_factor.flatten()) | np.isnan(y))
                if np.sum(valid_mask) < self.min_regression_weeks:
                    continue

                X_factor_clean = X_factor[valid_mask].reshape(-1, 1)
                y_clean = y[valid_mask]

                # Simple linear regression to get R²
                try:
                    model = LinearRegression()
                    model.fit(X_factor_clean, y_clean)
                    y_pred = model.predict(X_factor_clean)
                    r2 = r2_score(y_clean, y_pred)
                    factor_r2_scores[factor] = max(r2, 0)
                except:
                    factor_r2_scores[factor] = 0

            # Step 2: Select top 1-3 factors with highest R²
            if not factor_r2_scores:
                print(f"Warning: No valid factors for PC{pc_idx + 1} at {rebalance_date.date()}")
                results[pc_idx] = {
                    'model': None,
                    'scaler_X': None,
                    'scaler_y': None,
                    ...
                    'prediction': None,
                    'actual': None,
                    'model_type': 'No valid factors',
                    'cv_score': 0,
                    'coefficients': {}
                }
                continue

            # Sort factors by R² and select top 1-3
            sorted_factors = sorted(factor_r2_scores.items(), key=lambda x: x[1], reverse=True)
            best_r2 = sorted_factors[0][1]
            if best_r2 < 0.05:
                num_factors_to_select = 1
            elif best_r2 < 0.15:
                num_factors_to_select = min(2, len(sorted_factors))
            else:
                num_factors_to_select = min(3, len(sorted_factors))

            selected_factors = [factor for factor, r2 in sorted_factors[:num_factors_to_select]]
            selected_indices = [list(factor_returns_aligned.columns).index(factor) for factor in selected_factors]

            print(f"PC{pc_idx + 1}: Selected {len(selected_factors)} factors with R² scores:")
            for factor in selected_factors:
                print(f"  {factor}: {factor_r2_scores[factor]:.4f}")

            # Step 3: Build regression model with selected factors
            X = factor_returns_aligned[selected_factors].values
            ...
            if len(X_clean) < self.min_regression_weeks:
                print(f"Warning: Insufficient clean data for PC{pc_idx + 1} at {rebalance_date.date()}")
                results[pc_idx] = {
                    'model': None,
                    'scaler_X': None,
                    'scaler_y': None,
                    ...
                    'prediction': None,
                    'actual': None,
                    'model_type': 'Insufficient clean data',
                    'cv_score': 0,
                    'coefficients': {factor: 0 for factor in selected_factors}
                }
                continue
            ...
            # Cross-validation
            try:
                tscv = TimeSeriesSplit(n_splits=min(5, len(X_scaled) // 10))
                cv_scores = []
                ...
                cv_score = -np.mean([mean_squared_error(y_scaled[val_idx],
                                                        LinearRegression().fit(X_scaled[train_idx],
                                                                               y_scaled[train_idx]).predict(
                                                            X_scaled[val_idx]))
                                     for train_idx, val_idx in tscv.split(X_scaled)])
            except:
                cv_r2_mean = train_r2
                cv_r2_std = 0
                cv_score = -mean_squared_error(y_scaled, y_train_pred)

            # Make prediction using the latest factor data
            latest_factors = factor_returns_aligned[selected_factors].iloc[-1:].values
            pred_value = None
            if not np.isnan(latest_factors).any():
                try:
                    latest_scaled = scaler_X.transform(latest_factors)
                    pred_scaled = model.predict(latest_scaled)[0]
                    pred_value = scaler_y.inverse_transform([[pred_scaled]])[0, 0]
                except Exception as e:
                    print(f"Warning: Prediction failed for PC{pc_idx + 1} at {rebalance_date.date()}: {e}")

            # Get actual value if available
            actual_value = next_pc_returns[pc_idx] if next_pc_returns is not None and pc_idx < len(
                next_pc_returns) else None

            # Calculate test error if both prediction and actual are available
            test_error = None
            if pred_value is not None and actual_value is not None:
                test_error = abs(pred_value - actual_value) / (abs(actual_value) + 1e-10)

            # Store predictions and actuals
            if pred_value is not None:
                self.predictions[pc_idx][rebalance_date] = pred_value
            if actual_value is not None:
                self.actuals[pc_idx][rebalance_date] = actual_value

            # Create coefficients dictionary
            coef_dict = {factor: model.coef_[i] if i < len(model.coef_) else 0 for i, factor in
                         enumerate(selected_factors)}
            coef_dict['intercept'] = model.intercept_

            # Fill in zero coefficients for non-selected factors
            for factor in factor_returns_aligned.columns:
                if factor not in coef_dict:
                    coef_dict[factor] = 0
            ...
            results[pc_idx] = {
                ...
                'actual': actual_value,
                'model_type': f'Linear Regression (Top {len(selected_factors)} factors by R²)',
                'cv_score': cv_score,
    ...
    def plot_r2_scores(self):
        ...
        for rebalance_date in self.rebalance_dates:
            ...
            for pc_idx in range(self.num_pcs):
                ...
                train_r2_data[pc_idx].append(train_r2 if train_r2 is not None else np.nan)
                test_r2_data[pc_idx].append(test_r2 if test_r2 is not None else np.nan)
                print(f"PC{pc_idx + 1} at {rebalance_date.date()}: Train R²={train_r2}, Test R²={test_r2}")
        ...
        for pc_idx in range(self.num_pcs):
            ...
            print(f"PC{pc_idx + 1}: {len(valid_r2)} valid training R² points")
            if len(valid_r2) > 0:
                ax1.plot(valid_dates, valid_r2, label=f'PC{pc_idx + 1}', marker='o', markersize=3,
                         color=colors[pc_idx % len(colors)], alpha=0.7)
            else:
                print(f"Warning: No valid training R² data for PC{pc_idx + 1}")
        ...
        for pc_idx in range(self.num_pcs):
            valid_mask = ~np.isnan(test_r2_data[pc_idx])
            valid_dates = np.array(dates)[valid_mask]
            valid_r2 = np.array(test_r2_data[pc_idx])[valid_mask]
            print(f"PC{pc_idx + 1}: {len(valid_r2)} valid test R² points")
            if len(valid_r2) > 0:
                ax2.plot(valid_dates, valid_r2, label=f'PC{pc_idx + 1}', marker='o', markersize=3,
                         color=colors[pc_idx % len(colors)], alpha=0.7)
            else:
                print(f"Warning: No valid test R² data for PC{pc_idx + 1}")
        ...
        print("\nAverage Rolling Test R² over time:")
        for pc_idx in range(self.num_pcs):
            avg_r2 = np.nanmean(test_r2_data[pc_idx])
            print(f"PC{pc_idx + 1}: {avg_r2:.4f}")


    def update_comparison_metrics(self, rebalance_date):
        """Update metrics for plot comparisons (called weekly in main)."""
        r_hat = self.compute_predicted_stock_returns(rebalance_date)
        actual_returns = self.rebalance_data.get(rebalance_date, {}).get('actual_returns')
        if r_hat is not None and actual_returns is not None:
            # Average over time
            self.avg_predicted_over_time.append(np.mean(r_hat))
            self.avg_actual_over_time.append(np.mean(actual_returns))
            # Significant movements: |actual| > 2 * std (or customize threshold)
            std_actual = np.std(actual_returns)
            significant_mask = np.abs(actual_returns) > 2 * std_actual
            self.significant_predicted.extend(r_hat[significant_mask])
            self.significant_actual.extend(actual_returns[significant_mask])

    def plot_predicted_vs_actual_over_time_comparison(self):
        """Overlay plot of average predicted vs actual stock returns over time, with correlation output."""
        plt.figure(figsize=(12, 6))
        dates = self.rebalance_dates[:len(self.avg_predicted_over_time)]  # Align with data length
        plt.plot(dates, self.avg_predicted_over_time, label='Average Predicted Returns', color='blue')
        plt.plot(dates, self.avg_actual_over_time, label='Average Actual Returns', color='orange', alpha=0.7)
        plt.title('Average Predicted vs Actual Stock Returns Over Time')
        plt.xlabel('Rebalance Date')
        ...
        # Text output: Correlation between time series
        if len(self.avg_predicted_over_time) > 1:
            corr = np.corrcoef(self.avg_predicted_over_time, self.avg_actual_over_time)[0, 1]
            spike_coincidence = np.sum(
                np.sign(self.avg_predicted_over_time) == np.sign(self.avg_actual_over_time)) / len(
                self.avg_predicted_over_time)
            print(f"Time Series Correlation: {corr:.3f}")
            print(f"Spike Direction Coincidence Rate: {spike_coincidence:.2%}")
            print(
                "Interpretation: Positive correlation and high coincidence indicate similar spike timing, despite noise in actual returns.")
        else:
